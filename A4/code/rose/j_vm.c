...
*/
	...
	ioreqs = (ioreq_t *)heap_ptr;
	heap_ptr += MAX_IOREQS * sizeof(ioreq_t);
	...
	vm_sp = thr_active->curr_frame->sp;
 	...
 	/* remember method's frame so we know when to return from run() */
	  ...
	  /* establish the next time slice expiration time */
		...
		/* Check blocking io requests and see if there ms timeout is up. */
		int i = 0;
		for (; i < MAX_IOREQS; i++)
		{
			if (ioreqs[i].used && vm_time_ms >= ioreqs[i].vm_timeout)
			{
				ioreqs[i].used = false;
				IOComplete(i);
			}
