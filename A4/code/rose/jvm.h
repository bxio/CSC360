...
#define BLOCK_ON_IO		 10U		/**< waiting for IO completion */


/*
** Flags in ioreq_t.type field
*/
#define IOT_DISK_SEEK		1U 		/**< thread is disk seeking */
#define IOT_DISK_READ		2U		/**< thread is writing to disk */
#define IOT_DISK_WRITE		3U		/**< thread is reading from disk */
...
typedef _PACKED_ struct ioreq_T		 ioreq_t;
...
// struct ioreq_T {
	// bool      used;			/**< true if request is being used */
	// uint16    type;			/**< request type */
	// uint32    timeout;  	/**< request timeout period (0=none) */
	// int32     fd;			/**< file descriptor */
	// int32     param[4]; 	/**< IO buffer, seek position, etc */
// };

struct ioreq_T {
	bool		used;			/**< Used to check if this ioreq_t should be checked */
	long long	vm_timeout;		/**< How much time in ms before movement complete */
	thread_t 	*worker;		/**< Store a pointer to our worker thread */
};
...
/*===== Fields used by ROSE ===== */
	...
	mutex_t     *relock;            /**< a Mutex to be relocked after wake up from a Condition */
    //ioreq_t		 ioreq;				/**< the IO request this thread is waiting on */
