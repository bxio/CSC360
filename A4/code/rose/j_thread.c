...
bool InitROSE(void)
{
	...
	for (i=0; i < MAX_IOREQS; i++)
    {
        ioreqs[i].used = false;
        ioreqs[i].worker = NULL;
    }
...
/** Get the total number of threads that are active */
 ...
 */
 ...
 void IOBlock(int32 timeout)
 {
	ioreqs[0].vm_timeout = vm_time_ms + timeout;
	thr_active->state = BLOCK_ON_IO;
	ioreqs[0].worker = thr_active;
	ioreqs[0].used = true;
	Dispatch();
 }
 
 void IOComplete(int32 index)
 {
    AssignQuantum(ioreqs[index].worker);
	AddReady(ioreqs[index].worker, true);
	ioreqs[index].worker = NULL;
	PreemptIfNecessary();
 }
...
void CondWait( int32 cond_id, int32 mutex_id )
{
	...
	if (m->owner != thr_active) return; /* Not the owner is looking at mutex */
		
	/* Is anyone waiting? */
	if (m->blockQ != NULL) { 
		/* yes, take the first one as the new owner */
		DeQ( &(m->blockQ), &p );
		m->owner = p;
		AssignQuantum( p );
		AddReady( p, false );
		...
		m->owner = NULL; /* the mutex is free now! */

	/* Now, I am ready to wait on Condition "c" */
	c = ConditionOf( cond_id );
	thr_active->state = BLOCK_ON_COND;
	thr_active->relock = m;
	EnQ( &(c->blockQ), thr_active );
	/* Now, select a new running thread */
	Dispatch();
...
void CondSignal( int32 cond_id )
{
	...
	c = ConditionOf( cond_id );
	/* Is anyone waiting? */
	if (c->blockQ != NULL) { /* yes */
		DeQ( &(c->blockQ), &p );
		m = p->relock;
		/* Now, relock "m" for "p" if possible. */
		if ( m->owner == NULL ) {
			m->owner = p;
			AssignQuantum( p );
			AddReady( p, true );
		...
		else {  /* put "p" to be first in line waiting for "m" */
...
void CondBroadcast( int32 cond_id )
{
	...
	if (c->blockQ != NULL) { /* yes */
		pending = NULL;
		/* put all blocked threads in reverse order in a pending Q */
		while (c->blockQ != NULL) {
			DeQ( &(c->blockQ), &p );
			p->state = BLOCK_ON_MUTEX;
			AddFront( &pending, p );
		}
		/* now, resume them one at a time by placing them at the front
		* of the mutex blockQ; we have preserved the waiting order
		*/
		m = p->relock;
		while (pending != NULL) {
			DeQ( &pending, &p );
			AddFront( &(m->blockQ), p );
			if ( m != p->relock ) {
				fprintf( stderr, "ERROR: Not all threads wait on the same \
				mutex in a broadcast condition!\n" );
				exit(0);
			}
		}
		/* if the mutex is free, lock it */
		if ( m->owner == NULL ) {
			DeQ( &(m->blockQ), &p );
			m->owner = p;
			AssignQuantum( p );
